...
JOB_TIMEOUT = int(os.environ.get("SDC_JOB_TIMEOUT", 60 * 30))
...
job_interval = dcc.Interval(id="job-interval", interval=0.5 * 1000, disabled=True)
...
def sanitize_dataframe(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    report = {"dropped_columns": [], "date_columns": []}
    ...
    for col in df.columns:
        ...
        if pd.api.types.is_datetime64_any_dtype(series):
            ...
            if not samples.empty:
                ...
                parsed_samples = [val.strftime("%Y-%m-%d") for val in samples]
        ...
        if pd.api.types.is_string_dtype(series) or series.dtype == object:
            ...
            parsed_example = (
                parsed_non_null.iloc[0] if not parsed_non_null.empty else None
            )
            info["sample_value"] = (
                str(sample_original) if sample_original is not None else None
            )
            if parsed_example is not None:
                info["suggested_format"] = (
                    _infer_datetime_format(info["sample_value"]) or ""
                )
            ...
            if not parsed_non_null.empty:
                ...
                info["parsed_examples"] = [
                    val.strftime("%Y-%m-%d") for val in parsed_samples
                ]
...
def build_sanitization_alert(report: dict):
    ...
    for info in report.get("date_columns", []):
        ...
        if status == "converted":
            ...
            msg = (
                f"Column `{col}` left as text ({valid}/{total} values resembled dates)."
            )
            if invalid:
                sample = ", ".join(map(str, examples))
    ...
    alert_body = [html.Strong("Import notes"), html.Ul(messages, className="mb-0")]

    return dbc.Alert(alert_body, color="info", className="mt-3", dismissable=True)


def prepare_dataset_payload(
    original_df: pd.DataFrame, sanitized_df: pd.DataFrame, filename: str, report: dict
):
    report = report or {}
    ...
    for col in sanitized_serialized_df.columns:
        if pd.api.types.is_datetime64_any_dtype(sanitized_serialized_df[col]):
            sanitized_serialized_df[col] = (
                sanitized_serialized_df[col]
                .dt.tz_localize(None)
                .dt.strftime("%Y-%m-%d %H:%M:%S")
            )
    sanitized_serialized_df = sanitized_serialized_df.where(
        sanitized_serialized_df.notna(), ""
    )
    ...
    for col in original_serialized_df.columns:
        if pd.api.types.is_datetime64_any_dtype(original_serialized_df[col]):
            original_serialized_df[col] = (
                pd.to_datetime(original_serialized_df[col])
                .dt.tz_localize(None)
                .dt.strftime("%Y-%m-%d %H:%M:%S")
            )
    original_serialized_df = original_serialized_df.where(
        original_serialized_df.notna(), None
    )
...
def build_date_review_body(
    ...
    if errors:
        error_messages = [
            html.Li(f"{col}: {message}") for col, message in errors.items()
        ]
        rows.append(
            dbc.Alert(
                [
    ...
    for info in relevant:
        ...
        sample_pairs = list(
            zip(info.get("sample_values") or [], info.get("parsed_examples") or [])
        )
        ...
        if sample_pairs:
            ...
            samples_list = html.P(
                "No parsed examples available yet.", className="text-muted mb-0"
            )
                ...
                [
                    dbc.Col(
                        [
                            ...
                            html.Br(),
                            html.Small(
                                "First 5 parsed examples", className="text-muted"
                            ),
                        ],
                        width=4,
                    ),
                    ...
                    dbc.Col(
                        [
                            ...
                            html.Small(
                                "Leave blank to keep the inferred format.",
                                className="text-muted",
                            ),
                        ],
                        width=4,
...
ts1_dropdown = dcc.Dropdown(
    options=[{"label": "Add dataset to select", "value": "Empty"}],
    placeholder="Select time series 1",
    id="ts1-dropdown",
)
ts2_dropdown = dcc.Dropdown(
    options=[{"label": "Add dataset to select", "value": "Empty"}],
    placeholder="Select time series 2",
    id="ts2-dropdown",
)
    ...
    dbc.CardBody(
        [
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [html.Label("Date Column"), date_dropdown],
                        className="parameter-col",
                    ),
                    dbc.Col(
                        [html.Label("Time Series 1"), ts1_dropdown],
                        className="parameter-col",
                    ),
                    dbc.Col(
                        [html.Label("Time Series 2"), ts2_dropdown],
                        className="parameter-col",
                    ),
                    dbc.Col(
                        [html.Label("Corr. Method"), method_dropdown],
                        className="parameter-col",
                    ),
                ],
                ...
                [
                    dbc.Col(
                        [
                            html.Label("Min Lag"),
                            dbc.Input(
                                id="min-lag", placeholder="Default: -inf", type="number"
                            ),
                        ],
                        className="parameter-col",
                    ),
                    dbc.Col(
                        [
                            html.Label("Max Lag"),
                            dbc.Input(
                                id="max-lag", placeholder="Default: inf", type="number"
                            ),
                        ],
                        className="parameter-col",
                    ),
                ...
                [
                            ...
                            dbc.Col(
                                [
                                    ...
                                    dbc.Input(
                                        ...
                                        type="number",
                                        min=5,
                                        value=DEFAULT_PARAMS["labels_fontsize"],
                                    ),
                                ],
                                className="parameter-col",
                                width=4,
                    ...
                    dbc.Row(
                        [
                            ...
                            dbc.Col(
                                [
                                    html.Label("Plot Min Lag"),
                                    dbc.Input(
                                        id="plot-min-lag",
                                        type="number",
                                        placeholder="Use analysis min lag",
                                    ),
                                ],
                                className="parameter-col",
                                width=3,
                            ...
                            dbc.Col(
                                [
                                    html.Label("Plot Max Lag"),
                                    dbc.Input(
                                        id="plot-max-lag",
                                        type="number",
                                        placeholder="Use analysis max lag",
                                    ),
                                ],
                                className="parameter-col",
                                width=3,
            ...
            dbc.CardBody(
                [
                    ...
                    dcc.Upload(
                        id="upload-data",
                        children=html.Div(
                            "Upload dataset (.csv)", className="upload-box"
                        ),
                        className="upload-container",
                    ),
                    dbc.Button(
                        "Load example dataset: ONI anomalies vs T anomalies in South America 1998-2024",
        ...
        dbc.CardHeader(
            dbc.Button(
                [
                    ...
                    html.Span(
                        "▴", id="preview-card-icon", className="card-toggle-icon"
                    ),
                ],
...
plot_modal = dbc.Modal(
    [
        ...
        dbc.ModalBody(
            html.Img(id="modal-plot-img", style={"width": "100%", "height": "auto"})
        ),
...
def parse_contents(contents, filename):
    ...
    try:
        ...
        return (
            None,
            dbc.Alert(
                "Unsupported file type. Please upload a .csv file.", color="warning"
            ),
            None,
            None,
        )
        ...
        return (
            None,
            dbc.Alert(
                "There was an error processing this file. Please upload a valid .csv file.",
                color="danger",
            ),
            None,
            None,
    ...
    if trigger == "load-example-button":
        ...
        if not EXAMPLE_DATASET_PATH.exists():
            error_alert = dbc.Alert(
                "Example dataset could not be loaded. Please upload your own CSV file.",
                color="danger",
            )
            return (
                no_update,
        ...
        return prepare_dataset_payload(
            original_df, sanitized_df, EXAMPLE_DATASET_PATH.name, report
        )
    if trigger == "upload-data":
        if content is None:
            raise PreventUpdate
        sanitized_df, error, report, original_df = parse_contents(
            content, filename or "uploaded.csv"
        )
        ...
        return prepare_dataset_payload(
            original_df, sanitized_df, filename or "uploaded.csv", report
        )
    raise PreventUpdate
...
def update_series(data):
    if data is not None:
        ...
        # Default selections
        date_val = date_cols[0] if date_cols else None
...
def manage_date_review_modal(
    ...
    if trigger == "date-report-store":
        ...
        has_relevant = any(
            info.get("status") in {"converted", "datetime"} for info in date_infos
        )
    ...
    if trigger == "confirm-date-review":
        ...
        sanitized_serialized = sanitized_df.where(sanitized_df.notna(), "").to_dict(
            "list"
        )
...
def enqueue_sdc_job(
    ...
    if data is None:
        return {
            "status": "error",
            "message": "Please upload a dataset before running the analysis.",
        }
    ...
    try:
        n_permutations = (
            int(n_permutations)
            if n_permutations not in (None, "")
            else DEFAULT_PARAMS["n_permutations"]
        )
    except (TypeError, ValueError):
...
def manage_job(job_data, _):
    ...
    if trigger == "job-store":
        ...
        if job_data.get("status") == "queued":
            progress_children = build_progress_content(
                {"description": job_data.get("message"), "current": 0, "total": None}
            )
            return (
                progress_children,
                False,
    ...
    try:
        ...
        alert = dbc.Alert(
            "The analysis job could not be found. Please try again.", color="danger"
        )
        return (
            [alert],
            False,
    ...
    if job.is_failed:
        alert = dbc.Alert(
            "The analysis failed. Check the logs and try again.", color="danger"
        )
        return (
            [alert],
            False,
    ...
    if job.is_finished:
        ...
        if not result:
            alert = dbc.Alert(
                "No result was returned by the analysis.", color="warning"
            )
            return (
                [alert],
                False,
        ...
        download_button = dbc.Button(
            "Download", id="download-button", color="secondary"
        )
        xlsx_button = dbc.Button(
            "XLSX", id="download-results-xlsx-button", color="secondary", outline=True
        )
        png_button = dbc.Button(
            "PNG", id="download-plot-png-button", color="secondary", outline=True
        )
        svg_button = dbc.Button(
            "SVG", id="download-plot-svg-button", color="secondary", outline=True
        )
        ...
        results_children = [
            ...
            html.Small(
                "Click the plot to open a large preview.", className="plot-hint"
            ),
            image_div,
        ]

        total = progress.get("total") or 1
        success_progress = build_progress_content(
            {"description": "Completed", "current": total, "total": total}
        )
...
def on_update_plot(
    ...
    try:
        label_fontsize = (
            ...
            else DEFAULT_PARAMS["labels_fontsize"]
        )
    except (TypeError, ValueError):
    ...
    feedback = html.Span(
        "Plot updated with the latest styling options.", className="plot-feedback-text"
    )
    ...
    if updated_results:
        ...
        if "summary" in updated_results and isinstance(
            updated_results["summary"], dict
        ):
